...
def build_vision_tower(vision_tower_cfg, **kwargs):
    ...
    if is_absolute_path_exists or vision_tower.startswith("openai") or vision_tower.startswith("laion") or "ShareGPT4V" in vision_tower:
        print("loading CLIP model from openclip")
        if use_s2:
    ...
    elif vision_tower.startswith("open_clip_hub") or "CoCa" in vision_tower:
        print("loading COCA model from openclip")
        import open_clip
        coca_model, _, image_processor = open_clip.create_model_and_transforms(model_name="coca_ViT-L-14",pretrained="laion2b_s13b_b90k",precision="fp32",device="cuda")
        return COCAVisionTower(vision_tower, args=vision_tower_cfg, **kwargs)
    elif "CLIP-ViT" in vision_tower:
        print("loading CLIP model from openclip")
        return OpenCLIPVisionTower(vision_tower, args=vision_tower_cfg, **kwargs)
    # elif "coca" in vision_tower.lower():
    #     return COCAVisionTower(vision_tower, args=vision_tower_cfg, **kwargs)
