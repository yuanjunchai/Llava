...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        # rank0_print(f"image processor: {processor}, image aspect ratio: {image_aspect_ratio}")
        ...
        if image_aspect_ratio == "highres":
            ...
            image = processor(image)   ## For CoCA
        else:
            # image = processor.preprocess(image, return_tensors="pt")["pixel_values"][0] ## original
            # rank0_print(f"image from Open CLIP processor shape: {image.shape}")
            image = processor(image) ## For CoCA
            rank0_print(f"image from COCA processor shape: {image.shape}")
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        # rank0_print(f"Loading sample {i} with {len(sources['conversations'])} conversations")
        # rank0_print(sources)
        if isinstance(i, int):
            sources = [sources]
        assert len(sources) == 1, "Don't know why it is wrapped to a list"  # FIXME
        # rank0_print(f'source is : {sources[0]}')
        ...
        if "image" in sources[0]:
            ...
            try:
                if "shareVideoGPTV" in video_file:
                    ...
                    # breakpoint()
                    # print(f'video shape read by decord{video.shape}')
        ...
        if image is None:  # Check if process_image returns None
            print(f"Skipping sample {i} due to invalid image path.")
            return self._get_item(i + 1)
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "image" in instances[0]:
            ...
            # for x in range(len(images)):
                # print(f'dataloader : {x}, {images[x].shape}') #[B, N, C, H384, W384]
        ...
        # rank0_print(f'Batch lens: {len(batch)}')
        return batch
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    rank0_print(f'train_dataset: {train_dataset}')
    data_collator = DataCollatorForSupervisedDataset(tokenizer=tokenizer)
    rank0_print(f'data_collator: {data_collator}')
...
def train(attn_implementation=None):
    breakpoint()
    import open_clip
    coca_model, _, image_processor = open_clip.create_model_and_transforms(model_name="coca_ViT-L-14",pretrained="laion2b_s13b_b90k",precision="fp16",device="cuda")
    ...
    # breakpoint input/output, replace origin with our new one
        ...
        if data_args.image_grid_pinpoints is not None:
            if isinstance(data_args.image_grid_pinpoints, str) and "x" in data_args.image_grid_pinpoints:
                try:
                    ...
                    patch_size = data_args.image_processor.transforms[0].size
                except Exception as e:
